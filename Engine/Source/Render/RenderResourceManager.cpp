...
namespace RTGDEngine
{
    ...
    static MaterialData emptyMaterialMutable = {};
    ...
    void RenderResourceManager::Initialize(Diligent::IRenderDevice& device, Diligent::IDeviceContext& context)
    {
        ...
        m_textures.Slots.reserve(256);
        m_materials.Slots.reserve(256);
        m_meshes.Slots.reserve(256);
    ...
    void RenderResourceManager::BuildGBufferSRB(MaterialData& mat, Diligent::IPipelineState* pso)
    {
        using namespace Diligent;
        const TextureHandle def = Instance().GetDefaultTextureHandle();
        const TextureHandle defN = Instance().GetDefaultNormalTextureHandle();
        ...
        auto srvOf = [&](TextureHandle h, TextureHandle fb) -> ITextureView*
        {
            const TextureData* t = &Instance().GetTexture(h != INVALID_TEXTURE_HANDLE ? h : fb);
            if (!t->SRV)
                t = &Instance().GetTexture(fb);
            return t->SRV;
        };

        auto bindTex = [&](const char* name, TextureHandle h, TextureHandle fb)
        {
            if (auto* v = mat.SRB->GetVariableByName(SHADER_TYPE_PIXEL, name))
                v->Set(srvOf(h, fb));
        };

        bindTex("g_Diffuse", mat.DiffuseTexture, def);
        bindTex("g_Normal", mat.NormalTexture, defN);
        bindTex("g_MetallicRoughness", mat.MetallicRoughnessTexture, def);
        bindTex("g_AO", mat.AOTexture, def);
    ...
    MaterialData& RenderResourceManager::GetMaterial(MaterialHandle handle)
    {
        if (handle.Index() >= m_materials.size() || m_materials[handle.Index()].generation != handle.Generation())
        {
            LogError("Material handle generation missmatch.");
            return emptyMaterialMutable;
        }
    ...
    void RenderResourceManager::QueueMeshUpload(MeshHandle handle, std::vector<VertexPNTUV> vertices,
                                                std::vector<uint32_t> indices, const AABB& localBounds)
    {
    ...
    void RenderResourceManager::FlushMeshUploads(Diligent::IRenderDevice& device)
    {
        ...
        for (auto& upload: uploads)
        {
            ...
            data.LocalBounds = upload.LocalBounds;
