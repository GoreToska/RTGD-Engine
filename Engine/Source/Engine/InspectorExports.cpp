...
static std::string GetTypeName(flecs::world_t* world, ecs_entity_t type)
{
    ...
    if (prim)
    {
        switch (prim->kind)
        {
            case EcsF32:    return "float";
            case EcsF64:    return "double";
            case EcsI8:     return "int8";
            case EcsI16:    return "int16";
            case EcsI32:    return "int32";
            case EcsI64:    return "int64";
            case EcsU8:     return "uint8";
            case EcsU16:    return "uint16";
            case EcsU32:    return "uint32";
            case EcsU64:    return "uint64";
    ...
    const EcsStruct* s = static_cast<const EcsStruct*>(ecs_get(world, type, EcsStruct));
    if (s)
    {
...
static std::string ReadPrimValue(flecs::world_t* world,
                                 const void* ptr,
                                 ecs_entity_t typeId)
{
    const EcsPrimitive* prim = static_cast<const EcsPrimitive*>(
        ecs_get(world, typeId, EcsPrimitive));

    if (!prim) return "?";

    switch (prim->kind)
    {
        case EcsF32:    return std::to_string(*static_cast<const float*>(ptr));
        case EcsF64:    return std::to_string(*static_cast<const double*>(ptr));
        case EcsI8:     return std::to_string(*static_cast<const int8_t*>(ptr));
        case EcsI16:    return std::to_string(*static_cast<const int16_t*>(ptr));
        case EcsI32:    return std::to_string(*static_cast<const int32_t*>(ptr));
        case EcsI64:    return std::to_string(*static_cast<const int64_t*>(ptr));
        case EcsU8:     return std::to_string(*static_cast<const uint8_t*>(ptr));
        case EcsU16:    return std::to_string(*static_cast<const uint16_t*>(ptr));
        case EcsU32:    return std::to_string(*static_cast<const uint32_t*>(ptr));
        case EcsU64:    return std::to_string(*static_cast<const uint64_t*>(ptr));
        case EcsBool:   return *static_cast<const bool*>(ptr) ? "true" : "false";
        case EcsString:
        {
                             ...
                             std::vector<FieldInfo>& out)
{
    const EcsStruct* s = static_cast<const EcsStruct*>(
        ecs_get(world, typeId, EcsStruct));

    if (!s || !s->members.array) return;
    ...
    for (int32_t i = 0; i < s->members.count; i++)
    {
        ...
        const EcsStruct* nested = static_cast<const EcsStruct*>(
            ecs_get(world, m.type, EcsStruct));
        ...
        if (nested)
        {
            ...
            field.value    = "";
            if (memberPtr)
                ReadStructFields(world, memberPtr, m.type, field.children);
        }
        else
        {
            field.isStruct = false;
            field.value    = memberPtr
                ? ReadPrimValue(world, memberPtr, m.type)
                : "N/A";
        }
...
INSPECTOR_API int Inspector_GetComponentCount(uint64_t entityId)
{
    ...
    e.each([&](flecs::id id)
    {
        if (id.is_pair() || !id.is_entity()) return;
        ...
        ReadStructFields(world, compData, id.raw_id(), comp.fields);
...
{
    std::lock_guard lock(s_cacheMutex);
    if (index < 0 || index >= static_cast<int>(s_compCache.size())) return "";
    return s_compCache[index].name.c_str();
}

INSPECTOR_API int Inspector_GetFieldCount(int compIndex)
{
    if (compIndex < 0 || compIndex >= static_cast<int>(s_compCache.size())) return 0;
...
{
    if (compIndex < 0 || compIndex >= static_cast<int>(s_compCache.size())) return "";
...
INSPECTOR_API int Inspector_GetSubFieldCount(int compIndex, int fieldIndex)
{
    if (compIndex < 0 || compIndex >= static_cast<int>(s_compCache.size())) return 0;
    auto& fields = s_compCache[compIndex].fields;
    if (fieldIndex < 0 || fieldIndex >= static_cast<int>(fields.size())) return 0;
    return static_cast<int>(fields[fieldIndex].children.size());
}
...
INSPECTOR_API const char* Inspector_GetSubFieldName(int compIndex, int fieldIndex, int subIndex)
{
    ...
    auto& fields = s_compCache[compIndex].fields;
    if (fieldIndex < 0 || fieldIndex >= static_cast<int>(fields.size())) return "";
    auto& children = fields[fieldIndex].children;
    if (subIndex < 0 || subIndex >= static_cast<int>(children.size())) return "";
    return children[subIndex].name.c_str();
}
...
INSPECTOR_API const char* Inspector_GetSubFieldType(int compIndex, int fieldIndex, int subIndex)
{
    ...
    auto& children = fields[fieldIndex].children;
    if (subIndex < 0 || subIndex >= static_cast<int>(children.size())) return "";
    return children[subIndex].typeName.c_str();
}
...
INSPECTOR_API const char* Inspector_GetSubFieldValue(int compIndex, int fieldIndex, int subIndex)
{
    ...
    auto& children = fields[fieldIndex].children;
    if (subIndex < 0 || subIndex >= static_cast<int>(children.size())) return "";
    return children[subIndex].value.c_str();
}
