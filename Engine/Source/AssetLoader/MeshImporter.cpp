    ...
    MeshImportData MeshImporter::Import(const std::string &absolutePath) {
        ...
        for (uint32_t m = 0; m < scene->mNumMeshes; m++) {
            ...
            for (uint32_t i = 0; i < mesh->mNumVertices; i++) {
                ...
                result.LocalBounds.Max.x = std::max(result.LocalBounds.Max.x, vertex.Position.x);
                result.LocalBounds.Max.y = std::max(result.LocalBounds.Max.y, vertex.Position.y);
                result.LocalBounds.Max.z = std::max(result.LocalBounds.Max.z, vertex.Position.z);
                result.LocalBounds.Min.x = std::min(result.LocalBounds.Min.x, vertex.Position.x);
                result.LocalBounds.Min.y = std::min(result.LocalBounds.Min.y, vertex.Position.y);
                result.LocalBounds.Min.z = std::min(result.LocalBounds.Min.z, vertex.Position.z);
